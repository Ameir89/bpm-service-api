...
class Tasks:
    ...
    """
    def update_task(self, task_id, workflow_id, name ,task_type = None):
        if not task_id or not workflow_id or not name :
            return {"error": "task ID is required.", "success": False}
        ...
        try:
            ...
            General.write_event(f"Error while update task: {str(e)}")
            return {"error": f"Error while update  task: {str(e)}" ,"success" : False}
